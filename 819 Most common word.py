...
def mostCommonWord(paragraph, banned):
    ...
    freqList = {value: len(list(freq)) for value, freq in groupby(sorted(data))}
    #freqList = sorted(freqList.items(),key=lambda kv:(kv[1],kv[0]),reverse= True)
    print(freqList)
    maxValue = 0
    ans = ""
    for key,val in freqList:
        if val>maxValue and key not in banned:
            maxValue = val
            ans = key
    return ans
...
print(mostCommonWord(paragraph,["m","q","e","l","c","i","z","j","g","t","w","v","h","p","d","b","a","r","x","n"]))
